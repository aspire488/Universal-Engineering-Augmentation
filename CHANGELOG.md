...
- Deterministic-first philosophy — LLMs used only for strategy and ambiguity, not repeatable operations
- MCP-compatible — works alongside any MCP server

### Verification

- 15/15 core module imports validated
- 6/6 specialist integration tests passing
...
- 6 specialist system integration tests
- 16-check automated verification suite
- Deterministic behavior validation across all core modules

### Documentation

- Comprehensive README with architecture, capabilities, and quick start
- Benchmark report with measured performance data
- Release scorecard and audit documentation
- Specialist agents investigation report
...
- `semgrep` not bundled — static analysis verification tier requires separate installation
- `mutmut` requires WSL on Windows — mutation testing limited on native Windows
- Tree-sitter grammars bundled for Python, JavaScript, TypeScript, JSON only — Rust, Go, Java planned
- LLM token/call reduction not measured — augmentation provides capability expansion, not token savings
- No CI/CD at initial publication
- No unit tests at initial publication (6 integration tests only)
